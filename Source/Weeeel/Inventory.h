...
DECLARE_DYNAMIC_MULTICAST_SPARSE_DELEGATE(FInvUpd, UInventory, InventoryUpdate);
...
public:	
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, replicated, ReplicatedUsing = OnRep_Inventory)
		TArray<FItemSlot> Inventory;


	UFUNCTION()
		virtual void OnRep_Inventory(const TArray<FItemSlot>& OldStamina);

	UPROPERTY(EditAnywhere, BlueprintReadOnly)
		int GenerateCellOnStart;


	UPROPERTY(BlueprintAssignable, Category = "Components|Inventory")
		FInvUpd InventoryUpdate;
